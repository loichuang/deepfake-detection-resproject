...
def evaluate_and_save(pipeline, model, dataloader, criterion, device, scores_path, labels_path):
    ...
    with torch.no_grad():
        for crop, label in dataloader:
            ...
            from src.wenyi.train_ldm_try1 import extract_ldm_features
...
if __name__ == "__main__":
    ...
    celeb_dataset = CelebDFDataset(num_frames=2)   # 2 frames/video : ~1.5× plus rapide
    ...
    ffds_loss, ffds_acc, ffds_auc = evaluate_and_save(
        pipeline, model, ffds_loader, criterion, device,
        scores_path=str(RESULTS_DIR / "wenyi_try1_scores_ffpp.npy"),
        labels_path=str(RESULTS_DIR / "wenyi_try1_labels_ffpp.npy"),
    )

    print("\nSTARTING CELEB-DF CROSS-DATASET EVALUATION...")
    celeb_loss, celeb_acc, celeb_auc = evaluate_and_save(
        pipeline, model, celeb_loader, criterion, device,
        scores_path=str(RESULTS_DIR / "wenyi_try1_scores_celeb.npy"),
        labels_path=str(RESULTS_DIR / "wenyi_try1_labels_celeb.npy"),
    )
